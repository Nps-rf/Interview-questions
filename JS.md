...
### 5. Node.js — это однопоточный runtime. Как ты решаешь проблему с блокировкой основного потока (event loop), когда нужно выполнить долгие и ресурсоемкие операции?
**Ответ:** Использую асинхронные операции, `Worker Threads` для CPU-bound задач и внешние сервисы, такие как очереди RabbitMQ, для работы с длительными процессами.
